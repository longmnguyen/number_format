...
class NumberPlugin extends React.Component {
  ...
  getNumberRegex(g, ignoreDecimalSeparator = false) {
    const {format, precision} = this.props;
    const {decimalSeparator} = this.getSeparators();
    return new RegExp('\\d' + (decimalSeparator && precision !== 0 && !ignoreDecimalSeparator && !format ? '|' + escapeRegExp(decimalSeparator) : ''), g ? 'g' : undefined);
  }
  ...
  correctCaretPosition(value, caretPos, direction = false) {
  ...
  formatAsNumber(numStr) {
    const {precision, roundDecimal, prefix, suffix, allowNegative, thousandsGroupStyle} = this.props;
    const {thousandSeparator, decimalSeparator} = this.getSeparators();

    const hasDecimalSeparator = numStr.indexOf('.') !== -1 || (precision && roundDecimal);
    let {beforeDecimal, afterDecimal, addNegation} = splitDecimal(numStr, allowNegative); // eslint-disable-line prefer-const

    //apply decimal precision if its defined
    if (precision !== undefined) afterDecimal = limitToScale(afterDecimal, precision, roundDecimal);
  ...
  formatValueProp(defaultValue) {
    const {format, precision, roundDecimal, allowEmptyFormatting} = this.props;
    ...
    if (isNumericString && !format && typeof precision === 'number') {
      value = roundToPrecision(value, precision, roundDecimal)
    }
  ...
  isCharacterAFormat(caretPos, value) {
    const {format, prefix, suffix, precision, roundDecimal} = this.props;
    const {decimalSeparator} = this.getSeparators();
    ...
    if (!format && (caretPos < prefix.length
      ...
      || (precision && roundDecimal && value[caretPos] === decimalSeparator))
    ) {
      return true;
    }
  ...
  correctInputValue(caretPos, lastValue, value) {
    const {format, allowNegative, prefix, suffix, precision} = this.props;
  ...
  updateValue(params) {
    ...
    let {inputValue, numAsString, caretPos} = params;
  ...
  onKeyDown(e) {
    ...
    const {precision, roundDecimal, prefix, suffix, format, onKeyDown} = this.props;
    const ignoreDecimalSeparator = precision !== undefined && roundDecimal;
...
NumberPlugin.defaultProps = defaultProps;
NumberPlugin.propTypes = propTypes;
